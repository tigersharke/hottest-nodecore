...
local c_pumsponge = minetest.get_content_id("nc_pumsponge:pumsponge_glowing")
...
local function spawn(area, data, x, y, z, rng)
	...
	nodecore.scan_flood({x = x, y = y, z = z}, 5, function(p)
			...
			if data[idx] ~= c_pumsponge and data[idx] ~= c_sand then return false end
			idx = area:index(p.x, p.y, p.z)
			if data[idx] ~= c_lava then return false end
			data[idx] = c_pumsponge
			total = total + 1
